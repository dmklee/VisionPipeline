...
for i in range(7,11):
	if i == 8: continue
	DATA = np.loadtxt('data_%d.txt' % i, delimiter = ',')[:-5]
	...
	plt.plot(DATA[CORNERS,2], DATA[CORNERS,3], 'rs', markersize=5)
	plt.plot(DATA[END_CORNERS,2], DATA[END_CORNERS,3], 'r^', markersize=3)
	plt.plot(DATA[0,2], DATA[0,3], 'go', markersize=5)
	# plt.xlim((np.amin(DATA[:,2])-15, np.amax(DATA[:,2])+15))
	# plt.ylim((np.amin(DATA[:,3])-15, np.amax(DATA[:,3])+15))
	plt.axis('equal')
plt.show()
